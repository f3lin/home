...
# Téléchargement et installation de Oh My Posh
RUN bash -c "$(curl -fsSL https://ohmyposh.dev/install.sh)"

# Téléchargement du thème "Bubbles" de Oh My Posh depuis le référentiel GitHub
RUN curl -fsSL -o /root/simple.omp.yaml https://raw.githubusercontent.com/f3lin/my-terminal/main/themes/simple.yaml

# Configuration du shell Bash pour utiliser Oh My Posh
RUN echo 'eval "$(oh-my-posh --init --shell bash --config /root/simple.omp.yaml)"' >> ~/.bashrc
...
# Commande par défaut pour démarrer le shell Bash
CMD ["/bin/bash"]
